// В каждом варианте задания необходимо обеспечить выполнение
// следующих действий:
...
// данных. Число записей в файле - не менее 20. Поиск организовать по
// совпадению.
...
//------------------- creat(),  open()
#include <fcntl.h>     // описание параметров функций  определение O_RDONLY
#include <io.h>        // описание прототипов функций
#include <sys\types.h> // определение типов, используемых системой
...
int insert_record(int base, struct rec_base *rea, unsigned ins_pos, enum ins_mode mode);
int read_rec(int base, unsigned int pos);
int remote_reset(int base);
void remote_repair(int base);
void write_rec(int base, unsigned int pos, struct rec_base *rea);
...
int main(void)
{
    ...
    base = open("Протокол соревнований.dat", O_RDONLY);
    if (base == -1)
    {
        base = open("Протокол соревнований.dat", O_CREAT | O_BINARY, S_IREAD | S_IWRITE);
        close(base);

        base = open("Протокол соревнований.dat", O_RDWR | O_BINARY, S_IREAD | S_IWRITE);
    ...
    do
    {
        ...
        apostrophe(80);
        printf("\n");
        printf("\t    ***| Работа с файлами на уровне операционной системы |***\n\n");
...
void lineDateInTab(struct rec_base *buffer_basa, int pos)
{
    ...
    printf(("1. Организацация: %32s |           |       |"), buffer_basa->firm);
...
unsigned int get_rec_count(int base)
{
    ...
    lseek(base, 0, SEEK_END);
    long count_r = tell(base) / RECORD_S;
...
int insert_record(int base, struct rec_base *rea, unsigned ins_pos, enum ins_mode mode)
{
    ...
    {
        lseek(base, 0, SEEK_END);
        write(base, rea, RECORD_S); //?
        res = 1;
        return (res);
    }
    else // общий случай
    {
        base_n = open("Протокол соревнований_n.dat", O_CREAT | O_RDWR | O_BINARY, S_IREAD | S_IWRITE);
        lseek(base_n, 0, SEEK_SET);

        lseek(base, 0, SEEK_SET);
        for (i = 0; (!mode && (i < (int)ins_pos)) || (mode && (i <= (int)ins_pos)); i++)
        {
            read_c = read_rec(base, i);
            write(base_n, inp_zap, RECORD_S);
            free(inp_zap);
        }
        write(base_n, rea, RECORD_S);
        if (!mode)
            i = (int)ins_pos;
        else
            i = (int)ins_pos + 1;
        ...
        if (remove("Протокол соревнований.dat") != 0)
        {
            printf("\n\tНет возможности удалить основной файл базы данных перед перезаписью.\n");
            res = 0;
            return (res);
        }
        if (rename("Протокол соревнований_n.dat", "Протокол соревнований.dat") != 0)
        {
            printf("\n\tНет возможности переименовать вспомогательный файл базы данных в основной.\n");
            res = 0;
            return (res);
        }
        if (base = open("Протокол соревнований.dat", O_RDWR | O_BINARY, S_IREAD | S_IWRITE) == -1)
        {
            printf("\n\tНет возможности открыть основной файл базы данных.\n");
            res = 0;
            return (res);
        }
        return (res);
...
int read_rec(int base, unsigned int pos)
{
    int read_c;
    //отсчет pos с нуля
    lseek(base, RECORD_S * pos, SEEK_SET);
    inp_zap = (struct rec_base *)calloc(1, RECORD_S);
    read_c = read(base, inp_zap, RECORD_S);
...
int remote_reset(int base)
{
    ...
    return ((!remove("Протокол соревнований.dat")) && (!rename("Протокол соревнований_n.dat", "Протокол соревнований.dat")) && ((base = open("Протокол соревнований.dat", O_RDWR | O_BINARY, S_IREAD | S_IWRITE)) != -1));
}
void remote_repair(int base)
{
    unsigned rc;
...
void write_rec(int base, unsigned int pos, struct rec_base *rea)
{
    long cur_pos = tell(base);
    lseek(base, RECORD_S * pos, SEEK_SET);
    int write_c = write(base, rea, RECORD_S);
    if (write_c != 1)
        lseek(base, cur_pos, SEEK_SET);
    else
